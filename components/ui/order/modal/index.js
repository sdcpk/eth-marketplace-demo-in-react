...
export default function OrderModal({course, onClose, onSubmit, isNewPurchase}) {
  ...
  const _createFormState = (isDisabed = false, message = "") => ({isDisabed, message})
  ...
  const createFormState = ({price, email, confirmationEmail}, hasAgreedTOS, isNewPurchase) => {
    ...
    if (!hasAgreedTOS) {
      return _createFormState(true, "Youn need to agree with terms of service in order to submit the form")
    }
